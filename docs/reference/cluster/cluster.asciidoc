...
大部分集群层面的 API 允许明确方法在哪些节点上执行(比如说得到某个节点的统计值)。在 API 调用中可以通过节点 id，节点名称，节点地址，节点自定义特性或只是 `_local` 节点来明确接受请求的节点。 例如，下面是一些关于通过不同节点信息设置 API 执行的例子：
...
# 特性 (在 config 文件中设置例如 node.attr.rack: 2 这样的特性)
GET /_nodes/rack:2
GET /_nodes/ra*:2
GET /_nodes/ra*:2*
--------------------------------------------------
